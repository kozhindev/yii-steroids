...
export default () => WrappedComponent => class FileHoc extends React.PureComponent {
    ...
    initFiles(items) {
        this._uploader.queue.add(items.map(item => (new File({
    ...
    componentDidMount() {
        ...
        if (this.props.model && this.props.attribute && fileIds.length > 0 && !this.props.initialFiles) {
            store.dispatch(fetch(
                ...
                {
                    fileIds,
                    processor: this.props.imagesProcessor,
                }
            ));
        }
    }
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (!_isEqual(this.props.input.value, nextProps.input.value)) {
            const toRemove = _difference([].concat(this.props.input.value || []), [].concat(nextProps.input.value || []));
            if (toRemove.length > 0) {
                this._uploader.queue.remove(this._uploader.queue.getFiles().filter(file => {
                    return toRemove.indexOf(_get(file.getResultHttpMessage(), 'id')) !== -1;
                }));
            }
        }
    ...
    _onQueueItemEnd(file) {
        ...
        if (this.props.multiple) {
            this.props.input.onChange([].concat(this.props.input.value || []).concat([id]));
        } else {
